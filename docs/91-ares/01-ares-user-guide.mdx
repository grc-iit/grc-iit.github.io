...
Your system administrator can be reached via `nrajesh<at>iit<dot>edu`, and someone will get back to you.
...
2. Configure Lmod to be able to find those modules with:
   ...
   add this to your `~/.bashrc` to have it happen every time before `. spack/share/spack/setup-env.sh`
