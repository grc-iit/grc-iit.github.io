...
└── compressor
    ...
    ├── include
    │   └── compressor
    │       ├── compressor_client.h       # Client APIs (autogenerated)
    ...
    └── src
        ...
        └── compressor_runtime.temp_cc    # Runtime APIs (autogenerated)
```

## Bootstrap your new tasks

### Copy autogenerated tasks structs
...
To help reduce the burden of defining tasks, the file ``include/compressor/compressor_tasks.temp_h``
gets autogenerated and stores empty task definitions for each declared
in ``compressor_methods.yaml``.

To bootstrap tasks, copy-paste new task definitions
...
### Copy autogenerated client APIs
...
The autogenerated code is located at: ``include/compressor/compressor_client.temp_h``.
Copy ``Compress`` and ``Decompress`` into the ``compress.h`` file.

After copying, ``compressor_client.h`` should look like this:
```cpp
...
### Copy the autogenerated runtime APIs
...
A file named ``src/compressor_runtime.temp_cc`` is autogenerated containing the basic prototypes of the
